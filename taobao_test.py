...
def Get_Url(num):
    ...
    urlFront = 'https://rate.taobao.com/feedRateList.htm?auctionNumId=599885227219&userNumId=2086242818&currentPageNum='
    urlRear = '&pageSize=20&rateType=&orderType=sort_weight&attribute=&sku=&hasSku=false&folded=0&ua=098%23E1hvP9vUvbpvUvCkvvvvvjiPRssUsjEbPLS90jnEPmPyAjYEP25WsjibRscOsjlUiQhvCvvv9UUCvpvVvvpvvhCvkphvC99vvOCzp8yCvv9vvUm00ZT%2FKbyCvm9vvvvvphvvvvvvvZavpv1Ivvv2vhCv2UhvvUE3phvW9pvv9eHvpvACmphvLvA8O9vjRixrgj7r%2B3%2Biac7QD46Xjo2XS47BhC3%2Bm7zhaNpOHkyZ1W2WzC97nDeDyO2v5fh3Zi7vAWQaRoxBlwet9b8raoF6APL65dUPvpvhvv2MMsyCvvpvvvvv&_ksTS=1570239918427_1448&callback=jsonp_tbcrate_reviews_list'
...
def GetInfo(num):
    ...
    for i in range(num):
        ...
        nickname.extend(re.findall('"nick":"(.*?)"', content))  # 正则表达式匹配存入列表
        auctionSku.extend(re.findall('"sku":"(.*?)"', content))
        ratecontent.extend(re.findall('"content":"(.*?)"', content))
        ratedate.extend(re.findall('"date":"(.*?)"', content))
    # 将数据写入TEXT文件中
    for i in list(range(0, len(nickname))):
        text = ','.join((nickname[i], ratedate[i], auctionSku[i], ratecontent[i])) + '\n'
        with open(r"E:\TaoBaoContent.txt", 'a+', encoding='UTF-8') as file:
            file.write(text + ' ')
            print(i + 1, ":写入成功")
